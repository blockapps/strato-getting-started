...
### So you want to run a STRATO Node on a remote server?
...
```shell
bash <(curl -sSL https://raw.githubusercontent.com/blockapps/strato-getting-started/master/server-manager/update.sh)
```
or simply:
```shell
strato-update
...
```shell
bash <(curl -sSL https://raw.githubusercontent.com/blockapps/strato-getting-started/master/server-manager/ssl-get-cert.sh)
```
or simply:
```shell
ssl-get-cert
