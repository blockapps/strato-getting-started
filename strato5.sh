...
function pbftVote {
    ...
    docker exec --env NODEKEY="${blockstanbulPrivateKey}" strato_strato_1 bash -c "set -x && blockstanbul-vote $*"
}
...
then
  ...
  if [[ $(docker ps -a | grep strato_strato_1) ]]; then
    echo -e "${BYellow}Updating the existing STRATO Single node instance - getting it's blockstanbul keys...${NC}"
    ...
    echo -e "${BYellow}Running STRATO Single node from scratch - generating blockstanbul keys...${NC}"
    keys_json=$(keygen 1)
    ...
    export blockstanbulAdmins=${validators}
  fi
